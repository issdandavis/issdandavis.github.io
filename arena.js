var PLAYERS=[
    {id:'polly',name:'Polly',color:'#8fffd3',model:'scbe-backend',tongue:'DR',role:'Site Operator',avatar:'https://api.dicebear.com/7.x/bottts/svg?seed=polly'},
    {id:'pollinations',name:'Pollinations',color:'#ff6b6b',model:'openai',tongue:'AV',role:'Free Advocate',avatar:'https://api.dicebear.com/7.x/bottts/svg?seed=pollinations'},
...
function hasKey(id){if(id==='ollama'||id==='polly'||id==='pollinations')return true;var k=loadArenaKeys();return!!(k[id]&&k[id].trim())}
...
function orderedAvailableIds(){var a=availablePlayerIds(),pr=['google_ai','claude','xai','github_models','huggingface','ollama','groq','cerebras','openrouter','pollinations'];var r=pr.filter(function(id){return a.indexOf(id)!==-1});a.forEach(function(id){if(r.indexOf(id)===-1)r.push(id)});return r}
function narratorSeatId(){var ids=orderedAvailableIds();if(ids.indexOf('google_ai')!==-1)return'google_ai';if(ids.indexOf('polly')!==-1)return'polly';return ids.length?ids[0]:PLAYERS[0].id}
...
async function callBackendChat(message){var base=(window.__POLLY_BACKEND_HTTP__||'').replace(/\/$/,'');if(!base)throw new Error('Backend not configured');var resp=await fetch(base+'/v1/polly/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:message,context:'arena'})});var d=await resp.json();if(!resp.ok)throw new Error(d.error||'Backend error');return d?.data?.response||d?.response||''}
async function callPollinations(sys,usr){var prompt=sys+'\n\n'+usr;var resp=await fetch('https://text.pollinations.ai/',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:[{role:'system',content:sys},{role:'user',content:usr}],seed:Math.floor(Math.random()*999999),model:'openai'})});if(!resp.ok)throw new Error('Pollinations error: '+resp.status);return await resp.text()}
async function callProvider(pid,sys,usr,overrideKey){var p=PLAYERS.find(function(x){return x.id===pid}),model=p?p.model:'',key=overrideKey||getKey(pid);switch(pid){case'polly':return callBackendChat(sys+'\n\nUser: '+usr);case'pollinations':return callPollinations(sys,usr);case'groq':return callOpenAICompatible('https://api.groq.com/openai/v1/chat/completions',key,model,sys,usr);case'cerebras':return callOpenAICompatible('https://api.cerebras.ai/v1/chat/completions',key,model,sys,usr);case'xai':return callOpenAICompatible('https://api.x.ai/v1/chat/completions',key,model,sys,usr);case'openrouter':return callOpenAICompatible('https://openrouter.ai/api/v1/chat/completions',key,'moonshotai/kimi-k2-0711-preview',sys,usr,{'HTTP-Referer':window.location.origin,'X-Title':'AetherCode Arena'});case'github_models':return callOpenAICompatible('https://models.inference.ai.azure.com/chat/completions',key,model,sys,usr);case'google_ai':return callGoogle(key,model,sys,usr);case'claude':return callClaude(key,model,sys,usr);case'huggingface':return callHuggingFace(key,sys+'\n\n'+usr);case'ollama':return callOllama(model!=='local'?model:'llama3',sys,usr);default:throw new Error('Unknown provider: '+pid)}}
async function askPlayer(pid,message){var text=message||'';if(!text){var inp=document.getElementById('dealInput');if(inp)text=inp.value.trim()}if(!text)return null;if(!hasKey(pid)){var cfg=PROVIDER_KEY_MAP[pid];addMessage(pid,'error','No API key configured. Click "Keys" and enter your '+(cfg?cfg.envName:'key')+'.');return null}setStatus(pid,'thinking','thinking...');var player=PLAYERS.find(function(p){return p.id===pid}),ti=player?TONGUE_INSTRUCTIONS[player.tongue]||'':'',sys=player?'You are the '+player.role+' ('+player.tongue+' tongue) in the AI Round Table. '+ti:'';var t0=performance.now();try{var response=await callProvider(pid,sys,text);var lat=Math.round(performance.now()-t0);var meta=player?player.model+' / '+lat+'ms':'';if(pid==='polly')meta='SCBE Backend / '+lat+'ms';addMessage(pid,'ai',response,meta);setStatus(pid,'ready','ready');return{id:pid,name:player.name,tongue:player.tongue,role:player.role,response:response}}catch(err){var msg=err.message||'Request failed';if(msg.indexOf('Failed to fetch')!==-1||msg.indexOf('NetworkError')!==-1||msg.indexOf('CORS')!==-1)addMessage(pid,'error','Connection failed. Check your API key and network. For Ollama, ensure OLLAMA_ORIGINS=* is set.');else addMessage(pid,'error',msg);setStatus(pid,'error','error');setTimeout(function(){if(hasKey(pid))setStatus(pid,'ready','ready');else setStatus(pid,'nokey','no key')},5000);return null}}
...
async function relayChain(){var inp=document.getElementById('dealInput'),text=inp.value.trim();if(!text)return;inp.value='';var btn=document.getElementById('relayBtn');btn.disabled=true;btn.textContent='Relaying...';var ids=orderedAvailableIds().slice(0,6);if(!ids.length){ids=['polly']}var transcript=[],baton=text;for(var i=0;i<ids.length;i++){var res=await askPlayer(ids[i],'RELAY MODE\nOriginal: "'+text+'"\nBaton:\n'+trimText(baton,1200)+'\n\nAdd one useful step.\n1) Insight 2) Action 3) Handoff');if(res&&res.response){baton=res.response;transcript.push('['+res.name+'] '+trimText(res.response,900))}}if(transcript.length){var sid=narratorSeatId();var s=await askPlayer(sid,'RELAY TRANSCRIPT\n'+transcript.join('\n\n---\n\n')+'\n\nReturn a practical execution plan.');if(s&&s.response)addMessage(s.id||sid,'synthesis','[RELAY SYNTHESIS] '+s.response)}btn.disabled=false;btn.textContent='Relay'}
...
async function deliberate(){var inp=document.getElementById('dealInput'),text=inp.value.trim();if(!text)return;inp.value='';var btn=document.getElementById('deliberateBtn');btn.disabled=true;btn.textContent='Deliberating...';var ids=orderedAvailableIds();if(!ids.length){ids=['polly']}var results=await Promise.allSettled(ids.map(function(id){return askPlayer(id,text)}));var responses=results.filter(function(r){return r.status==='fulfilled'&&r.value}).map(function(r){return'['+r.value.name+' / '+r.value.tongue+' / '+r.value.role+']:\n'+r.value.response}).join('\n\n---\n\n');if(responses){var synthId=narratorSeatId();var s=await askPlayer(synthId,'ROUND TABLE SYNTHESIS\nAsked: "'+text+'"\n'+ids.length+' responded:\n\n'+responses+'\n\nSynthesize: 1. Agreement 2. Disagreement 3. Recommendation 4. Action items');if(s)addMessage(s.id||synthId,'synthesis','[ROUND TABLE CONSENSUS] '+s.response)}btn.disabled=false;btn.textContent='Deliberate'}
